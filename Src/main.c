...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
#define isInterrupt 0 // X-NUCLEO-53L1A1* If isInterrupt = 1 then device working in interrupt mode, else device working in polling mode */
/* USER CODE END PM */
...
void My_I2C1_Init(void) {
    ...
    /* 1. Enable the Clocks */
    __HAL_RCC_GPIOB_CLK_ENABLE(); // Enable clock for Port B (pins PB8/PB9)
    __HAL_RCC_I2C1_CLK_ENABLE();  // Enable clock for I2C1 peripheral

    /* 2. Configure GPIO Pins (PB8=SCL, PB9=SDA) */
    GPIO_InitStruct.Pin = GPIO_PIN_8 | GPIO_PIN_9;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_OD;       // Open Drain is required for I2C
    GPIO_InitStruct.Pull = GPIO_PULLUP;           // Internal pull-ups (helper, but external strongly recommended)
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    GPIO_InitStruct.Alternate = GPIO_AF4_I2C1;    // Connect pin to I2C1 hardware
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

    /* 3. Configure the I2C Peripheral */
    hi2c1.Instance = I2C1;
    hi2c1.Init.ClockSpeed = 400000;               // 400kHz (Fast Mode) - Important for VL53L1X
    ...
    if (HAL_I2C_Init(&hi2c1) != HAL_OK) {
        // Initialization Error - Add your error handler here (e.g., blink LED)
        while(1);
        printf("error initializing: I2C1\n");
    }
}
