...
#else
    /* STM32L4xx and L053 */
    hi2c.Init.Timing = 0x00300F38; /* set 400KHz fast mode i2c*/
#endif
...
static int _ExpanderRd(int I2cExpAddr, int index, uint8_t *data, int n_data) {
    ...
    do {
        status = HAL_I2C_Master_Transmit(&hi2c, I2cExpAddr, &RegAddr, 1, 100);
        if (status)
            break;
        status = HAL_I2C_Master_Receive(&hi2c, I2cExpAddr, data, n_data, n_data * 100);
    } while (0);
...
static int _ExpanderWR(int I2cExpAddr, int index, uint8_t *data, int n_data) {
    ...
    XNUCLEO53L1A1_GetI2cBus();
    status = HAL_I2C_Master_Transmit(&hi2c, I2cExpAddr, RegAddr, n_data + 1, 100);
    XNUCLEO53L1A1_PutI2cBus();
    return status;
}
